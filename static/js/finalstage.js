...
// Function for preparing final stage
function prepareFinalGame() {
    $('#game-container').empty().append(`
        <div id="final-game-container" class="text-center">
            <h2>Final Stage: Lightsaber, Force Shield, Blaster!</h2>
            <p>Through the power of your acquired artefacts, you have been granted ${userLives + wonChapters.length} extra lives</p>
            <p>After a hard fought battle to conquer the mind, Darth Sidious is the only thing that stands between you and saving the galaxy!</p>
...
function playFinalStage(userMove) {
    const moves = ['lightsaber', 'force-shield', 'blaster'];
    const bossMove = moves[Math.floor(Math.random() * moves.length)];

    $('#boss-choice').text(`Darth Sidious used: ${bossMove}`);

    if (userMove === bossMove) {
        $('#rps-result').text('Your attacks cancel eachother out! Try again.');
        return;
    }

    if ((userMove === 'force-shield' && bossMove === 'blaster') ||
        (userMove === 'blaster' && bossMove === 'lightsaber') ||
        (userMove === 'lightsaber' && bossMove === 'force-shield')) {
        // User won
        bossLife--;
        if (bossLife === 0) {
            winGame();
...
function winGame() {
    ...
    $('#back-to-menu').click(function() {
        ...
        window.location.href = afterChaptersLinkLink; // Redirect to home
    });
}
...
function gameOver() {
    ...
    `);
    $('#back-to-menu').click(function() {
        window.location.href = mainMenuLink; // Redirect to the main menu
    });
}
...
function initiateFinalStage() {
    userLives += wonChapters.length; // Add extra lives based on chapters completed
    startRPSGame();
}